...
class GetDataTablemodel extends CI_Model
{
    ...
    {
        $sql = "SELECT * FROM tb_prefix_name WHERE  prefix_name_id IN('005', '002', '004', '054', '055', '056', '504', '505', '506', '507', '508')ORDER BY prefix_name_id ASC ";
    ...
    public function form_dropdown_tb_faculty()
    {
        $sql = "SELECT * FROM tb_faculty ORDER BY faculty_id ASC ";
        $tb_religion = $this->db->query($sql)->result_array();

        $array = [];
        foreach ($tb_religion as $key => $item) {
            $id = $item['faculty_id'];
            $array[$id] = $item['faculty_name_th'];
        }
